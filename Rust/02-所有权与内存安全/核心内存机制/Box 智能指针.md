...
`Box<T>` 通常读作 "Box of T", 是 Rust 中最简单的 **智能指针**, 它的核心功能只有一个: 将数据存储在**堆**上, 而不是栈上.

### 1.1 核心特性

`Box<T>` 对它所指的堆上数据拥有 **独占所有权**, 这意味着一个数据智能有一个 Box 指向它;

当 `Box<T>` 本身离开其作用域时, 会自动调用 `drop` 方法释放指向堆的内存, 这保证了绝对不会发生内存泄漏;

`Box<T>` 实现了 `Deref` 和 `DerefMut` Trait, 所以你可以像使用普通引用一样, 使用 `*` 操作符来访问它指向的数据;
...
如果希望创建一个集合, 里面存放着不同类型, 但是实现了同一个 Trait 的示时, 需要使用 `Box`;
