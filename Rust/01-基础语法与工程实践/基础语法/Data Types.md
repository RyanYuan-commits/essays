...
type: Rust
sub-type: Basic
created: 2025-10-05 15:58:25
updated: 2025-10-05 18:00:59
---
## 1 bool
...
Rust 提供了多种整型, 用于表示整数. 整型分为有符号 (signed) 和无符号 (unsigned) 两种.

- **有符号整型**: 以 `i` 开头, 可以表示正数, 负数和零. (例如 `i8`, `i32`, `i64`)
- **无符号整型**: 以 `u` 开头, 只能表示非负数. (例如 `u8`, `u32`, `u64`)

数字后缀可以用来显式指定类型, 例如 `57u8`.
...
Rust 的 `char` 类型用于表示单个 **Unicode** 字符, 大小为 4 个字节.
