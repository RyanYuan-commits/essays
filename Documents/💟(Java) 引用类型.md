...
创建一个对象并把这个对象赋给一个引用变量 (我们用的最多的等号+new关键字), 如果内存不足, JVM 会抛出 OOM 错误也不会回收对象,  想中断强引用和某个对象之间的关联, 可以显示地将引用赋值为null. 
...
在上面的案例中, 我们将堆中的启始内存和最大内存都设置为 50MB, 然后构建一个强引用的对象；然后创建一个 8MB 的数组到堆中(内存溢出), 此时使用 finally 来查看 a 是什么, 可以看到即使内存溢出, 触发的 GC 也不会将 a 引用的对象清除. 
...
public static void main(String[] args) throws InterruptedException {  
    ...
    // 检查引用队列中是否有虚引用对象  
    Reference<?> removedRef = referenceQueue.poll();  
    if (removedRef != null) {  
        System.out.println("对象已被回收，虚引用已加入队列: " + removedRef);  
        System.out.println("此时可以执行资源清理等操作");  
    }  
}
```