# 为什么需要 undo log？
当我们在执行一条 CUD （增删改）操作的时候，虽然没有输入 begin 开启事务和 commit 提交事务，但是 MySQL 还是会隐式的开启事务来执行这样的语句，执行完就自动的提交事务，这样就保证了执行完一条修改数据的语句的时候，我们可以立刻在数据库表中看到结果。通过下面的 SQL 语句，我们可以配置全局或者当前会话的配置值。
```sql
...
```
将其设置为关闭的时候，我们再执行 SQL 语句就不会主动提交了，而是将所有的 SQL 语句放在一个 Transaction（事务）中，需要手动执行 `COMMIT` 语句来提交：
```sql
SET autocommit = 0;
...
COMMIT;
```
当执行完 INSERT 语句，库里是这样的光景，而在执行了 COMMIT 语句后，`jack` 才会被真正的添加进去
![[autocommit 演示.png]]
**我们都知道事务可以回滚，那回滚的数据是从哪里来的呢？**
