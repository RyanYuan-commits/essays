比如现在有一个这样的简单需求，地推员输入用户的姓名和手机号，系统根据客户的手机号的归属地和所属运行商，分配给响应的销售组，由销售组来进行后续的业务。
# 平平无奇的案例
```Java
public class User {
    Long userId;
...
4. 存储用户数据
>[!question] 上面的代码存在哪些问题呢？
>1. 方法的入参是两个 String，在方法被编译引用后，我们并不能通过方法的参数类型来确定我们的参数顺序，这就导致了接口语义的不明确，或者有可能因为开发者的纰漏导致了入参的填写错误。
...
```
我们可以将上面这个 PhoneNumber 类称为一个 DP（Domain Primitive），正如 String、Integer 是语言的基础，DP 是 DDD 架构中一切模型、方法和架构的基础；它是在特定的领域，拥有精确的定义、可以进行自我验证，用用行为的对象，可以认为是领域的最小组成部分。