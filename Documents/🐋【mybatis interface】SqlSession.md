## 接口定义
`SqlSession` 是 `MyBatis` 中的一个核心接口，它提供了与数据库交互的主要方法。通过 `SqlSession`，开发者可以执行 SQL 语句、管理事务、获取映射器等操作。

其提供了与数据库交互的基础方法、获取接口代理 Mapper 的方法、与事务有关的，例如 commit、rollback 方法。
